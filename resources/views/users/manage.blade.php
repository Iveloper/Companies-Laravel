...
<div class="form-group row" style="text-align: center">
        <h1><label for="role">{{trans('company.choose_role')}}  {{$roleName[0]->name }} </label></h1>
        <br>
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                        @foreach($permissionsByGroup as $permission)
                            {!! Form::checkbox('permissionForAdmin[]', $permission->id, in_array($permission->id, $permissions['rolePermissions']), ['class' => 'form-control']) !!} {{ ucfirst($permission->name) }}<br>
                        @endforeach
                 </div>
            </div>
        @endforeach
