...
{{ Form::hidden('p_id', $edit[0]->id) }}
<div class='form' style='text-align: center'>
    <div class="form-group row">
        <label for="name">{{trans('company.name')}}</label><br>
        {!! Form::text('p_name', $edit[0]->name, array('class'=>"form-control")) !!}
    </div>

    <div class="form-group row">
        <label for="email">{{trans('company.email')}}</label><br>
        {!! Form::text('p_email', $edit[0]->email, array('class'=>"form-control")) !!}
    </div>

    <div class="form-group row">
       <label for="address">{{trans('company.adress')}}</label><br>
        {!! Form::text('p_adress', $edit[0]->adress, array('class'=>"form-control")) !!}
    </div>
    
    <div class="form-group row">
        <label for="phone">{{trans('company.phone')}}</label><br>
        {!! Form::text('p_phone', $edit[0]->phone, array('class'=>"form-control")) !!}
    </div>
    
       <div class="form-group row">
        <label for="birthday">{{trans('company.birthday')}}</label><br>
        <input type="text" name="p_date" value="{{ $edit[0]->date_of_birth }}" id="date" autocomplete="off">
    </div>
